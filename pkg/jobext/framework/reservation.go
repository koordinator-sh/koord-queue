...
func (d *GenericJobReconciler) CreateOrUpdateReservation(ctx context.Context, log logr.Logger, resvs, existing []koordinatorschedulerv1alpha1.Reservation) (bool, error) {
	...
	for _, r := range resvs {
		...
		if !ok {
			...
			go func(r koordinatorschedulerv1alpha1.Reservation) {
				defer wg.Done()
				if err := d.client.Create(ctx, &r); err != nil && !errors.IsAlreadyExists(err) {
			...
			go func(r koordinatorschedulerv1alpha1.Reservation) {
				...
				if err := d.client.Update(ctx, &r); err != nil {
...
func (d *GenericJobReconciler) CreateReservations(ctx context.Context, log logr.Logger, resvs []koordinatorschedulerv1alpha1.Reservation) error {
	wg := sync.WaitGroup{}
	l := sync.Mutex{}
	errs := []error{}
	for _, r := range resvs {

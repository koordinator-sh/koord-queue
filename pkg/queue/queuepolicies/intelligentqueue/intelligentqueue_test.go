...
// TestFifoLess tests FIFO comparison function
func TestFifoLess(t *testing.T) {
	now := time.Now()
	...
	tests := []struct {
		...
		{
			name: "earlier timestamp should come first",
			a: &framework.QueueUnitInfo{
				Name:                    "default/job1",
				Unit:                    &v1alpha1.QueueUnit{ObjectMeta: metav1.ObjectMeta{UID: "job1"}},
				InitialAttemptTimestamp: earlier,
			},
			b: &framework.QueueUnitInfo{
				Name:                    "default/job2",
				Unit:                    &v1alpha1.QueueUnit{ObjectMeta: metav1.ObjectMeta{UID: "job2"}},
				InitialAttemptTimestamp: later,
			},
			expected: -1,
		},
		{
			name: "later timestamp should come after",
			a: &framework.QueueUnitInfo{
				Name:                    "default/job1",
				Unit:                    &v1alpha1.QueueUnit{ObjectMeta: metav1.ObjectMeta{UID: "job1"}},
				InitialAttemptTimestamp: later,
		...
		{
			name: "same timestamp, alphabetical by name",
			a: &framework.QueueUnitInfo{
				Name:                    "default/a",
				Unit:                    &v1alpha1.QueueUnit{ObjectMeta: metav1.ObjectMeta{UID: "a"}},
				InitialAttemptTimestamp: now,
			},
			b: &framework.QueueUnitInfo{
				Name:                    "default/b",
				Unit:                    &v1alpha1.QueueUnit{ObjectMeta: metav1.ObjectMeta{UID: "b"}},
				InitialAttemptTimestamp: now,
			},
			expected: -1,
		},

...
[![Build Status](https://app.travis-ci.com/koord-queue/koord-queue.svg?token=CNioeKg6kJ72Zcc3ZpeQ&branch=main)](https://app.travis-ci.com/koord-queue/koord-queue)

# Koord-queue
koord-queue is designed to manage AI/ML and batch workloads in Kubernetes. It allows system admins to customize job queue management for queues to provide flexibility and fairness between different queues. Combined with a quota system (similar to Kubernetes resource quota), koord-queue automates and optimizes workload and resource quota management to maximize cluster resource utilization.
...
```shell
$ git clone https://github.com/koordinator-sh/koord-queue.git
```

2. Change to koord-queue directory:
```shell
$ cd koord-queue
```

3. Deploy koord-queue with Helm
```shell
$ helm install koord-queue -n kube-system ./charts/v0.0.1
NAME: koord-queue
...
4. Check running status of koord-queue
```shell
$ helm get manifest koord-queue  -n kube-system | kubectl get -n koord-queue -f -
NAME                   STATUS   AGE
namespace/koord-queue   Active   2m17s

NAME                        SECRETS   AGE
serviceaccount/koord-queue   1         2m16s

NAME                                                                           CREATED AT
customresourcedefinition.apiextensions.k8s.io/queueunits.scheduling.x-k8s.io   2021-09-13T02:15:36Z

NAME                                               CREATED AT
clusterrole.rbac.authorization.k8s.io/koord-queue   2021-09-13T02:15:36Z

NAME                                                                         ROLE                     AGE
clusterrolebinding.rbac.authorization.k8s.io/koord-queue-clusterrolebinding   ClusterRole/koord-queue   2m16s

NAME                                        READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/koord-queue-controller       1/1     1            1           2m16s
deployment.apps/tf-operator-extension       1/1     1            1           2m17s
deployment.apps/pytorch-operator-extesion   1/1     1            1           2m17s
```

5. Uninstall koord-queue with Helm
```shell
$ helm uninstall koord-queue -n kube-system
```

### Example

We will submit two tensorflow (tf) jobs to the cluster at the same time, but the current cluster can only meet the resource requests of one job so one job is running and the other job is queued by koord-queue, and the pods of the queued job are not created.
